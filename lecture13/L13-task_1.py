...
Cu ajutorul acestei clase trebuie să fiu capabil să creez câte obiecte doresc cu orice configurație a numelui, prețului și anul_producerii
...
telefon = Produs("Iphone", 15000, 2020)
print(telefon.numele)
print(telefon.pretul)
print(telefon.anul_producerii)
...
class Telefon(Produs):
    ...
    def upgrade_memory(self, new_memory):
        ...
        print(f"Memoria a fost actualizată la {new_memory} GB.")

    def upgrade_battery(self, new_battery):
        self.baterie_mAh = new_battery
        print(f"Bateria a fost actualizată la {new_battery} mAh.")
telefon = Telefon("Samsung Galaxy S20", 4000, 2020, 3500, 8)
print(telefon.memorie_GB)
telefon.upgrade_memory(12)
print(telefon.memorie_GB)
# CODUL TĂU VINE MAI SUS:
...
class Laptop(Produs):
    ...
    def upgrade_processor(self, new_processor):
        ...
        print(f"Procesorul a fost actualizat la {new_processor}.")

    def upgrade_os(self, new_os):
        self.sistem_de_operare = new_os
        print(f"Sistemul de operare a fost actualizat la {new_os}.")
# CODUL TĂU VINE MAI SUS:
...
class Trotineta(Produs):
    ...
    def upgrade_speed(self, new_speed):
        ...
        print(f"Viteza maximă a fost actualizată la {new_speed} km/h.")

    def upgrade_autonomy(self, new_autonomy):
        self.autonomie_km = new_autonomy
        print(f"Autonomia a fost actualizată la {new_autonomy} km.")
# CODUL TĂU VINE MAI SUS:
...
class AppleProduct(Produs):
    ...
    def combine_products(self, product):
        if isinstance(product, AppleProduct) and product.culoare == self.culoare:
            ...
            print("Produsul a fost conectat cu succes.")
        else:
            print("Produsul nu poate fi conectat deoarece culorile nu coincid.")

iphone = AppleProduct("Iphone", 15000, 2020, "negru")
airpods = AppleProduct("Airpods", 1000, 2021, "alb")
iphone.combine_products(airpods)

iphone2 = AppleProduct("Iphone", 15000, 2020, "negru")
airpods2 = AppleProduct("Airpods", 1000, 2021, "negru")
iphone2.combine_products(airpods2)
print(iphone2.produs_conectat.numele)
# CODUL TĂU VINE MAI SUS:
...
class GoogleProduct(AppleProduct):
    def combine_products(self, product):
       if isinstance(product, GoogleProduct):
            ...
            return "Produsul nu poate fi conectat deoarece nu este un produs Google."

pixel = GoogleProduct("Pixel", 10000, 2020, "negru")
home = GoogleProduct("Home", 500, 2021, "alb")
pixel.combine_products(home)
print(pixel.produs_conectat.numele)
# CODUL TĂU VINE MAI SUS:
